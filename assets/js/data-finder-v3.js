...
(function () {
  ...
  function readyFitsFilename(item, index) {
    var target = safeFilenamePart(item.targetLabel || 'Target') || 'Target';
    var filter = (item.filters && item.filters.length === 1)
      ? safeFilenamePart(normaliseFilterName(item.filters[0]))
      : ('Channel' + (index + 1));
    var colour = safeFilenamePart(item.channelColor || '');
    return target + '_' + filter + (colour ? '_' + colour.toUpperCase() : '') + '.fits';
  }

  function readyFitsStem(item, index) {
    return readyFitsFilename(item, index).replace(/\.fits$/i, '');
  }
  ...
  function renderBasket() {
    ...
    if (items.length === 0) {
      ...
      basketList.innerHTML = items.map(function (item, index) {
        var finalName = readyFitsFilename(item, index);
        var meta = [item.telescope, item.instrument, (item.filters || []).join(', '), item.channelColor || '', 'ready-to-process SCI image']
          .filter(Boolean).join(' · ');
        return '<li class="basket-item">' +
          '<div class="basket-item-info">' +
            '<span class="basket-item-name">' + escapeHtml(finalName) + '</span>' +
            '<span class="basket-item-meta">' + escapeHtml(meta) + '</span>' +
          '</div>' +
          '<div class="product-actions basket-item-actions">' +
            '<button type="button" class="basket-remove" data-index="' + index + '" aria-label="Remove ' + escapeHtml(finalName) + ' from basket">&times;</button>' +
          '</div>' +
        '</li>';
      }).join('');
    }
  ...
  function downloadPowerShellScript() {
    ...
    var lines = [
      "# Ready-to-process Hubble/JWST FITS downloader generated by Our Astro Journey.",
      "# MAST files are downloaded temporarily; Siril saves the first SCI image",
      "# as one simple FITS per colour channel, then the temporary files are deleted.",
      "$ErrorActionPreference = 'Stop'",
      "$ProgressPreference = 'SilentlyContinue'",
      "$workDir = (Get-Location).Path",
      "$siril = $null",
      "$cmd = Get-Command siril-cli.exe -ErrorAction SilentlyContinue",
      "if ($cmd) { $siril = $cmd.Source }",
      "if (-not $siril) {",
      "  $candidates = @(",
      "    (Join-Path $env:ProgramFiles 'Siril\\bin\\siril-cli.exe'),",
      "    (Join-Path $env:LOCALAPPDATA 'Programs\\Siril\\bin\\siril-cli.exe')",
      "  )",
      "  $siril = $candidates | Where-Object { Test-Path $_ } | Select-Object -First 1",
      "}",
      "if (-not $siril) {",
      "  Write-Host 'Siril is required to create the simple SCI FITS files automatically.' -ForegroundColor Red",
      "  Write-Host 'Install Siril, then run this script again.'",
      "  exit 1",
      "}",
      ""
    ];
    ...
    items.forEach(function (item, index) {
      var finalName = readyFitsFilename(item, index);
      var finalStem = readyFitsStem(item, index);
      var tempName = '.oaj_source_' + (index + 1) + '.fits';
      var filterLabel = (item.filters && item.filters.length) ? item.filters.join(', ') : ('Channel ' + (index + 1));
      var colourLabel = item.channelColor ? ' ' + item.channelColor : '';
    ...
    lines.push("Write-Host 'Ready-to-process FITS files complete.' -ForegroundColor Green");

    downloadTextFile('oaj-ready-fits.ps1', lines.join('\r\n') + '\r\n', 'text/plain');
    track('manifest_generated', { format: 'ps1-ready-fits', file_count: items.length });
    setStatus('Your ready-to-process FITS PowerShell script has been generated.');
  }
  ...
  function requestManifest(format) {
    ...
    if (format === 'sh') {
      var lines = [
        ...
        'set -euo pipefail',
        'workdir="$(pwd)"',
        'siril="$(command -v siril-cli || true)"',
        'if [ -z "$siril" ] && [ -x "/Applications/SiriL.app/Contents/MacOS/siril-cli" ]; then siril="/Applications/SiriL.app/Contents/MacOS/siril-cli"; fi',
        'if [ -z "$siril" ]; then echo "Siril is required to create the simple SCI FITS files automatically."; exit 1; fi',
        ''
      ];
      ...
      items.forEach(function (item, index) {
        ...
        lines.push('save "' + finalStem + '"');
        lines.push('close');
        lines.push('OAJ_SIRIL');
        lines.push('[ -f ' + JSON.stringify(finalName) + ' ] || { echo ' + JSON.stringify('Could not create ' + finalName) + '; exit 1; }');
        lines.push('rm -f ' + JSON.stringify(tempName));
        lines.push('');
      });
      lines.push('echo "Ready-to-process FITS files complete."');
      downloadTextFile('oaj-ready-fits.sh', lines.join('\n') + '\n', 'text/x-sh');
      track('manifest_generated', { format: 'sh-ready-fits', file_count: items.length });
      setStatus('Your ready-to-process FITS Mac/Linux script has been generated.');
      return;
    }
      ...
      '<div class="group-actions">' +
        ...
        ((safePlan && matched)
          ? '<button type="button" class="button group-add-recommended" data-group="' + index + '">' +
              '<i class="fas fa-plus" aria-hidden="true"></i> Check & add colour set' +
            '</button>'
          : '<button type="button" class="button button-secondary group-add-recommended group-add-unavailable" data-group="' + index + '" disabled aria-disabled="true" title="No verified three-filter colour set was found on the same pointing">' +
              '<i class="fas fa-ban" aria-hidden="true"></i> No safe colour set' +
            '</button>') +
      '</div>' +
  ...
  function renderProducts(container, group, result, mode) {
    ...
    container.dataset.products = JSON.stringify(products.map(function (p) {
      return {
        ...
        hasPolarizer: !!p.hasPolarizer, channelColor: p.channelColor || ''
      };
    }));
  }
  ...
  function normaliseProductResults(data, obsids, group) {
    ...
    selectedIds.forEach(function (id, idIndex) {
      ...
      if (chosen) {
        ...
        chosen.channelColor = ['BLUE', 'GREEN', 'RED'][idIndex] || '';
  ...
  function finishRecommendedAdd(groupIndex, addBtn, box) {
    ...
    var added = addToBasket(products, { telescope: group.telescope, instrument: group.instrument, targetLabel: currentSearch.resolved.label });
  ...
  resultsEl.addEventListener('click', function (event) {
    ...
    if (singleAdd) {
      ...
      var addedOne = addToBasket([product], { telescope: g.telescope || '', instrument: g.instrument || '', targetLabel: currentSearch.resolved.label });
      if (addedOne > 0) {
